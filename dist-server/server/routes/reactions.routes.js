...
reactions.post('/', async (c) => {
    ...
    if (!['LIKE', 'DISLIKE'].includes(reactionType))
        throw new AppError(400, 'Неверный тип реакции');
    ...
    if (existing) {
        if (existing.type === reactionType) {
            ...
            if (type === 'COURSE')
                await prisma.course.update({ where: { id: reactionableId }, data: { likesCount: { decrement: reactionType === 'LIKE' ? 1 : 0 } } });
            else if (type === 'LESSON')
                await prisma.lesson.update({ where: { id: reactionableId }, data: { likesCount: { decrement: reactionType === 'LIKE' ? 1 : 0 } } });
    ...
    if (type === 'COURSE')
        await prisma.course.update({ where: { id: reactionableId }, data: { likesCount: { increment: reactionType === 'LIKE' ? 1 : 0 } } });
    else if (type === 'LESSON')
        await prisma.lesson.update({ where: { id: reactionableId }, data: { likesCount: { increment: reactionType === 'LIKE' ? 1 : 0 } } });
...
reactions.delete('/', async (c) => {
    ...
    if (!reaction)
        throw new AppError(404, 'Реакция не найдена');
    await prisma.reaction.delete({ where: { id: reaction.id } });
    if (type === 'COURSE')
        await prisma.course.update({ where: { id }, data: { likesCount: { decrement: reaction.type === 'LIKE' ? 1 : 0 } } });
    else if (type === 'LESSON')
        await prisma.lesson.update({ where: { id }, data: { likesCount: { decrement: reaction.type === 'LIKE' ? 1 : 0 } } });
