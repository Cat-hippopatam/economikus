...
admin.patch('/users/:id', async (c) => {
    ...
    if (role)
        await prisma.user.update({ where: { id: profile.userId }, data: { role } });
...
admin.post('/courses', async (c) => {
    ...
    const course = await prisma.course.create({
        data: { title, slug, description, coverImage, difficultyLevel: difficultyLevel || 'BEGINNER', duration: duration || 0, lessonsCount: 0, modulesCount: 0, isPremium: isPremium || false, status: status || 'DRAFT', authorProfileId: profile.id, tags: tagIds ? { create: tagIds.map((tagId) => ({ tagId })) } : undefined },
...
admin.patch('/courses/:id', async (c) => {
    const id = c.req.param('id');
    const { title, slug, description, coverImage, difficultyLevel, duration, isPremium, status, tagIds } = await c.req.json();
    if (slug && await prisma.course.findFirst({ where: { slug, NOT: { id } } }))
        return c.json({ error: 'Курс с таким slug уже существует' }, 400);
    if (tagIds !== undefined) {
        await prisma.courseTag.deleteMany({ where: { courseId: id } });
        if (tagIds.length > 0)
            await prisma.courseTag.createMany({ data: tagIds.map((tagId) => ({ courseId: id, tagId })) });
    }
...
admin.get('/modules', async (c) => {
    ...
    if (!courseId)
        return c.json({ error: 'courseId обязателен' }, 400);
...
admin.delete('/modules/:id', async (c) => {
    ...
    if (!module)
        return c.json({ error: 'Модуль не найден' }, 404);
...
admin.post('/lessons', async (c) => {
    ...
    if (await prisma.lesson.findUnique({ where: { slug } }))
        return c.json({ error: 'Урок с таким slug уже существует' }, 400);
    ...
    const lesson = await prisma.lesson.create({
        data: { moduleId, title, slug, description, lessonType: lessonType || 'ARTICLE', duration: duration || 0, isPremium: isPremium || false, status: status || 'DRAFT', sortOrder: sortOrder ?? (maxOrder?.sortOrder ?? 0) + 1, authorProfileId: profile.id, tags: tagIds ? { create: tagIds.map((tagId) => ({ tagId })) } : undefined },
        include: { tags: { include: { tag: true } } }
    });
...
admin.patch('/lessons/:id', async (c) => {
    ...
    if (slug && await prisma.lesson.findFirst({ where: { slug, NOT: { id } } }))
        return c.json({ error: 'Урок с таким slug уже существует' }, 400);
    if (tagIds !== undefined) {
        await prisma.lessonTag.deleteMany({ where: { lessonId: id } });
        if (tagIds.length > 0)
            await prisma.lessonTag.createMany({ data: tagIds.map((tagId) => ({ lessonId: id, tagId })) });
    }
...
admin.post('/tags', async (c) => {
    ...
    if (await prisma.tag.findUnique({ where: { slug } }))
        return c.json({ error: 'Тег с таким slug уже существует' }, 400);
...
admin.patch('/tags/:id', async (c) => {
    const id = c.req.param('id');
    const { name, slug, color } = await c.req.json();
    if (slug && await prisma.tag.findFirst({ where: { slug, NOT: { id } } }))
        return c.json({ error: 'Тег с таким slug уже существует' }, 400);
