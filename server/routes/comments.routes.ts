...
comments.get('/', optionalAuth, async (c) => {
  ...
  const where = { commentableType: type, commentableId: id, deletedAt: null, status: 'APPROVED' as const }
...
comments.post('/', async (c) => {
  ...
  if (text.length > 2000) throw new AppError(400, 'Комментарий слишком длинный (макс. 2000 символов)')
