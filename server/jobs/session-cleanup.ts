 ...
 */
export async function limitUserSessions(userId: string, maxSessions: number = 5): Promise<number> {
  try {
 ...
 */
export async function optimizeSessions(): Promise<{
  expired: number
  limited: number
}> {
  ...
  const usersWithManySessions = await prisma.$queryRaw<{ userId: string; count: bigint }[]>`
