...
| Модуль | Статус |
|--------|--------|
| Аутентификация | ✅ Реализовано (register, login, logout, me) |
| API курсов | 🔄 Заглушки (файлы созданы, код закомментирован) |
| API уроков | 🔄 Заглушки |
| API пользователя | 🔄 Заглушки |
...
| Линтинг           | ESLint                                                                |
| Иконки            | Lucide React, @tabler/icons-react                                    |
...
### 4.2 Курсы 🔄 Заглушка (файл создан, код закомментирован)

| № | Метод | Путь | Описание | Доступ |
|---|-------|------|----------|--------|
| 5 | GET | `/api/courses` | Список курсов с пагинацией | Все |
...
| 9 | POST | `/api/courses` | Создание курса | AUTHOR, ADMIN |
| 10 | PATCH | `/api/courses/:id` | Обновление курса | AUTHOR, ADMIN |
| 11 | DELETE | `/api/courses/:id` | Удаление курса | AUTHOR, ADMIN |

### 4.3 Уроки 🔄 Заглушка

| № | Метод | Путь | Описание | Доступ |
|---|-------|------|----------|--------|
| 12 | GET | `/api/lessons` | Список уроков | Все |
| 13 | GET | `/api/lessons/:slug` | Детальная страница урока | Все |
| 14 | GET | `/api/lessons/:slug/content` | Контент урока (текст/видео/аудио) | Все |
| 15 | POST | `/api/lessons` | Создание урока | AUTHOR, ADMIN |

### 4.4 Пользователь 🔄 Заглушка

| № | Метод | Путь | Описание | Доступ |
|---|-------|------|----------|--------|
| 16 | GET | `/api/user/me` | Текущий пользователь | Авторизованный |
| 17 | PATCH | `/api/user/profile` | Обновление профиля | Авторизованный |
| 18 | GET | `/api/user/profile/:nickname` | Публичный профиль | Все |
| 19 | GET | `/api/user/history` | История просмотров | Авторизованный |
| 20 | POST | `/api/user/history` | Добавить в историю | Авторизованный |
| 21 | GET | `/api/user/favorites` | Избранное | Авторизованный |
| 22 | POST | `/api/user/favorites` | Добавить в избранное | Авторизованный |
| 23 | DELETE | `/api/user/favorites/:id` | Удалить из избранного | Авторизованный |
| 24 | GET | `/api/user/progress/courses` | Прогресс по курсам | Авторизованный |
| 25 | GET | `/api/user/progress/lessons/:id` | Прогресс по уроку | Авторизованный |
| 26 | POST | `/api/user/progress/lessons/:id` | Обновить прогресс | Авторизованный |
| 27 | GET | `/api/user/certificates` | Сертификаты | Авторизованный |

### 4.5 Реакции 🔄 Заглушка

| № | Метод | Путь | Описание | Доступ |
|---|-------|------|----------|--------|
| 28 | GET | `/api/reactions` | Получить реакции | Все |
| 29 | POST | `/api/reactions` | Поставить реакцию | Авторизованный |
| 30 | DELETE | `/api/reactions` | Удалить реакцию | Авторизованный |

### 4.6 Теги 🔄 Заглушка

| № | Метод | Путь | Описание | Доступ |
|---|-------|------|----------|--------|
| 31 | GET | `/api/tags` | Список тегов | Все |
| 32 | GET | `/api/tags/:slug/courses` | Курсы по тегу | Все |
| 33 | GET | `/api/tags/:slug/lessons` | Уроки по тегу | Все |

### 4.7 Комментарии 🔄 Заглушка

| № | Метод | Путь | Описание | Доступ |
|---|-------|------|----------|--------|
| 34 | GET | `/api/comments` | Список комментариев | Все |
| 35 | POST | `/api/comments` | Добавить комментарий | Авторизованный |
| 36 | PATCH | `/api/comments/:id` | Редактировать комментарий | Авторизованный |
| 37 | DELETE | `/api/comments/:id` | Удалить комментарий | Авторизованный |
...
| № | Метод | Путь | Описание | Доступ |
|---|-------|------|----------|--------|
| 41 | GET | `/api/moderation` | Контент на модерации | MODERATOR, ADMIN |
| 42 | POST | `/api/moderation/:id/approve` | Одобрить контент | MODERATOR, ADMIN |
| 43 | POST | `/api/moderation/:id/reject` | Отклонить контент | MODERATOR, ADMIN |

### 4.10 Администрирование 🔄 Заглушка

| № | Метод | Путь | Описание | Доступ |
|---|-------|------|----------|--------|
| 44 | GET | `/api/admin/users` | Список пользователей | ADMIN |
| 45 | PATCH | `/api/admin/users/:id` | Изменить пользователя | ADMIN |
| 46 | DELETE | `/api/admin/users/:id` | Удалить пользователя | ADMIN |
| 47 | GET | `/api/admin/stats` | Статистика системы | ADMIN |
...
На данный момент реализованы только страницы аутентификации. Основная страница (`/`) отображает список пользователей из БД.

**Реализованные страницы:**

| № | Путь | Компонент | Описание | Статус |
|---|------|-----------|----------|--------|
| 1 | `/` | `src/App.tsx` | Главная (список пользователей) | ✅ |
...
|---|------|-----------|----------|
| 4 | `/catalog` | `src/pages/CatalogPage.tsx` | Каталог курсов и уроков |
| 5 | `/courses` | `src/pages/CoursesPage.tsx` | Список курсов |
| 6 | `/course/:slug` | `src/pages/CoursePage.tsx` | Детальная страница курса |
| 7 | `/course/:slug/module/:moduleId` | `src/pages/ModulePage.tsx` | Страница модуля |
| 8 | `/lesson/:slug` | `src/pages/LessonPage.tsx` | Страница урока |
| 9 | `/faq` | `src/pages/FaqPage.tsx` | Вопросы и ответы |
| 10 | `/tools` | `src/pages/ToolsPage.tsx` | Инструменты |
| 11 | `/calculator/credit` | `src/pages/CalculatorPage.tsx` | Кредитный калькулятор |
| 12 | `/calculator/deposit` | `src/pages/CalculatorPage.tsx` | Калькулятор вкладов |
| 13 | `/calculator/mortgage` | `src/pages/CalculatorPage.tsx` | Ипотечный калькулятор |

### 5.3 Планируемые защищённые маршруты

| № | Путь | Компонент | Описание | Требование |
|---|------|-----------|----------|------------|
| 14 | `/profile` | `src/pages/ProfilePage.tsx` | Личный кабинет | Авторизация |
| 15 | `/profile/courses` | `src/pages/ProfileCoursesPage.tsx` | Мои курсы | Авторизация |
| 16 | `/profile/favorites` | `src/pages/ProfileFavoritesPage.tsx` | Избранное | Авторизация |
| 17 | `/profile/settings` | `src/pages/ProfileSettingsPage.tsx` | Настройки профиля | Авторизация |
| 18 | `/profile/subscription` | `src/pages/ProfileSubscriptionPage.tsx` | Подписка | Авторизация |
| 19 | `/profile/history` | `src/pages/ProfileHistoryPage.tsx` | История просмотров | Авторизация |
| 20 | `/profile/certificates` | `src/pages/ProfileCertificatesPage.tsx` | Мои сертификаты | Авторизация |

### 5.4 Планируемые админ-маршруты

| № | Путь | Компонент | Описание | Требование |
|---|------|-----------|----------|------------|
| 21 | `/admin` | `src/pages/admin/AdminPage.tsx` | Главная админ-панель | ADMIN |
| 22 | `/admin/users` | `src/pages/admin/AdminUsersPage.tsx` | Управление пользователями | ADMIN |
| 23 | `/admin/content` | `src/pages/admin/AdminContentPage.tsx` | Управление контентом | ADMIN |
| 24 | `/admin/moderation` | `src/pages/admin/AdminModerationPage.tsx` | Модерация контента | MODERATOR, ADMIN |
| 25 | `/admin/stats` | `src/pages/admin/AdminStatsPage.tsx` | Статистика | ADMIN |

### 5.5 Планируемые автор-маршруты

| № | Путь | Компонент | Описание | Требование |
|---|------|-----------|----------|------------|
| 26 | `/author/dashboard` | `src/pages/author/AuthorDashboardPage.tsx` | Панель автора | AUTHOR, ADMIN |
| 27 | `/author/courses/new` | `src/pages/author/AuthorCourseNewPage.tsx` | Создание курса | AUTHOR, ADMIN |
| 28 | `/author/courses/:id/edit` | `src/pages/author/AuthorCourseEditPage.tsx` | Редактирование курса | AUTHOR, ADMIN |
...
❌ Модальные окна не реализованы. Планируется использование Mantine Modal.

### 8.2 Планируемая система модальных окон

Проект будет использовать централизованную систему модальных окон на основе:
- **Zustand** (`store/useModalStore.ts`) — глобальное состояние
- **ModalProvider** — рендер активной модалки

### 8.3 Планируемые модальные окна
...
### 10.3 Планируемая функциональность управления пользователями
...
- `server/lib/auth.ts` — конфигурация (закомментирована, резерв для @hono/auth-js)
...
| `src/store/useAppStore.ts` | Zustand стор | ✅ |
...
- [ ] Добавить ProtectedRoute компонент
- [ ] Реализовать `/profile` страницу
- [ ] Подключить остальные роуты API
- [ ] Добавить Header/Footer компоненты

### Приоритет 2 (Контент)
- [ ] Реализовать API курсов
- [ ] Реализовать API уроков
