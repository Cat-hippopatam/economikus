...
- [x] Профиль пользователя (`/user/:nickname`)
- [x] Настройки профиля (`/profile/settings`)
- [x] Смена пароля (`PATCH /user/password`)
- [ ] Избранное
- [ ] История просмотров
- [ ] Прогресс обучения
- [ ] Сертификаты

### Фаза 3: Авторская система

- [x] Заявка на статус автора (`/become-author`)
- [x] Модель AuthorApplication в БД
- [x] API для подачи заявки (`POST /author/apply`)
- [x] API для получения заявки (`GET /author/application`)
- [x] Панель автора (`/author/dashboard`)
- [x] Админ: управление заявками (`/admin/applications`)
- [x] Одобрение/отклонение заявок (`PATCH /admin/applications/:id`)
- [ ] Создание курсов автором
- [ ] Редактирование курсов автором
- [ ] Управление уроками
...
## 13. Реализованный функционал: Профиль и Авторская система

> **Дата реализации:** Январь 2025

### 13.1 Обзор реализованных функций

| Функция | Статус | Маршрут | Описание |
|---------|--------|---------|----------|
| Профиль пользователя | ✅ | `/user/:nickname` | Публичная страница профиля |
| Настройки профиля | ✅ | `/profile/settings` | Редактирование профиля |
| Смена пароля | ✅ | `PATCH /user/password` | Изменение пароля |
| Заявка на автора | ✅ | `/become-author` | Форма подачи заявки |
| Панель автора | ✅ | `/author/dashboard` | Статистика и управление |
| Админ: заявки | ✅ | `/admin/applications` | Модерация заявок |

### 13.2 Изменения в базе данных

#### Новая модель: AuthorApplication
...
#### Обновлённая модель Profile

```prisma
model Profile {
  // ... существующие поля ...
  
  authorApplications     AuthorApplication[]  @relation("ProfileApplications")
  reviewedApplications   AuthorApplication[]  @relation("ReviewedApplications")
}
```

### 13.3 Новые API endpoints

#### Авторские роуты (`server/routes/author.routes.ts`)

| Метод | Путь | Описание |
|-------|------|----------|
| GET | `/author/application` | Получить текущую заявку пользователя |
...
#### Обновлённые админ-роуты (`server/routes/admin.routes.ts`)

| Метод | Путь | Описание |
|-------|------|----------|
| GET | `/admin/applications` | Список заявок с пагинацией |
| PATCH | `/admin/applications/:id` | Одобрить/отклонить заявку |

#### Обновлённые пользовательские роуты (`server/routes/user.routes.ts`)

| Метод | Путь | Описание |
|-------|------|----------|
| PATCH | `/user/password` | Смена пароля |

### 13.4 Фронтенд страницы

#### ProfilePage (`src/pages/profile/ProfilePage.tsx`)

**Функционал:**
- Обложка профиля (градиент или изображение)
- Аватар с эффектом наложения
- Информация: displayName, nickname, bio
- Социальные ссылки: website, telegram, youtube
- Статистика: просмотры, курсы, уроки
- Вкладки: О себе, Курсы, Сертификаты
- Кнопка "Стать автором" для USER
- Кнопка редактирования для владельца

**UI-особенности:**
- `marginBottom: -50` у обложки
- `marginTop: -60` у аватара
- Эффект наложения аватара на обложку

#### ProfileSettingsPage (`src/pages/profile/ProfileSettingsPage.tsx`)

**Функционал:**
- Редактирование displayName, bio
- Социальные ссылки: website, telegram, youtube
- Загрузка аватара (UI готов, логика заглушка)
- Смена пароля с проверкой текущего
- Вкладки: Профиль, Пароль, Уведомления

#### BecomeAuthorPage (`src/pages/profile/BecomeAuthorPage.tsx`)

**Состояния:**
1. **Не авторизован** → редирект на `/login`
2. **Уже автор** → сообщение "Вы уже автор!"
3. **Заявка на рассмотрении** → статус PENDING
4. **Заявка отклонена** → причина + возможность подать снова
5. **Форма подачи** → мотивация (минимум 50 символов), опыт, портфолио

**Валидация:**
- Мотивация: минимум 50 символов
- Опыт: необязательно
- Портфолио: необязательно, URL

#### AuthorDashboardPage (`src/pages/author/AuthorDashboardPage.tsx`)

**Функционал:**
- Статистика: просмотры, студенты, курсы, уроки
- Быстрые действия: Мои курсы, Мои уроки
- Кнопка "Создать курс"

**Доступ:**
- AUTHOR, MODERATOR, ADMIN
- USER → редирект на `/become-author`

#### AdminApplications (`src/pages/admin/AdminApplications.tsx`)

**Функционал:**
- Таблица заявок с пагинацией
- Фильтрация по статусу (PENDING, APPROVED, REJECTED)
- Модальное окно с деталями заявки
- Одобрение → смена роли на AUTHOR
- Отклонение → обязательное указание причины

### 13.5 Навигация

#### Обновлённый Header (`src/components/layout/Header.tsx`)
...
│ Аккаунт                      │
├─────────────────────────────┤
│ 👤 Мой профиль               │ → /user/:nickname
│ ⚙️ Настройки                 │ → /profile/settings
│ ✏️ Стать автором             │ → /become-author (только USER)
│ 📝 Панель автора             │ → /author/dashboard (AUTHOR+)
├─────────────────────────────┤
...
### 13.6 Хук useAuth (`src/hooks/useAuth.ts`)

**Создан новый хук для управления аутентификацией:**
...
## 14. Трек на сегодня и план дальнейшего развития

### 14.1 Трек на сегодня (Тестирование)

**Приоритет: Высокий**

| Задача | Статус | Описание |
|--------|--------|----------|
| Тест смены пароля | 🔄 | Проверить `PATCH /user/password` |
| Тест подачи заявки | 🔄 | Создать заявку на автора |
| Тест одобрения заявки | 🔄 | Одобрить заявку админом |
| Тест отклонения заявки | 🔄 | Отклонить заявку с причиной |
| Тест панели автора | 🔄 | Проверить статистику |
| Проверка прав доступа | 🔄 | USER не может в /author/dashboard |

**Как тестировать:**

1. **Смена пароля:**
   - Войти как пользователь
   - Перейти в `/profile/settings`
   - Вкладка "Пароль"
   - Ввести текущий и новый пароль
   - Проверить, что вход работает с новым паролем

2. **Заявка на автора:**
   - Войти как USER
   - Нажать "Стать автором" в меню
   - Заполнить форму (мотивация > 50 символов)
   - Отправить заявку
   - Проверить статус "На рассмотрении"

3. **Одобрение заявки:**
   - Войти как ADMIN
   - Перейти в `/admin/applications`
   - Найти заявку в статусе PENDING
   - Нажать "Одобрить"
   - Проверить, что роль пользователя изменилась на AUTHOR

4. **Панель автора:**
   - Войти как AUTHOR
   - Перейти в `/author/dashboard`
   - Проверить статистику

### 14.2 План после тестирования

#### Фаза 1: Создание курсов автором (1-2 недели)

**Задачи:**
- [ ] Страница создания курса (`/author/courses/new`)
- [ ] Форма создания курса (title, description, difficulty, tags)
- [ ] API `POST /author/courses`
- [ ] Страница редактирования курса (`/author/courses/:id/edit`)
- [ ] API `PATCH /author/courses/:id`
- [ ] Управление модулями курса
- [ ] Управление уроками в модулях

**Новые маршруты:**
```
/author/courses           → Список курсов автора
/author/courses/new       → Создание курса
/author/courses/:id       → Просмотр курса
/author/courses/:id/edit  → Редактирование курса
/author/lessons           → Список уроков автора
/author/lessons/new       → Создание урока
```

#### Фаза 2: Публикация контента (2-3 недели)

**Задачи:**
- [ ] Модерация контента (PENDING_REVIEW)
- [ ] Workflow публикации: DRAFT → PENDING_REVIEW → PUBLISHED
- [ ] Уведомления авторам о статусе модерации
- [ ] Редактор контента уроков (Markdown/Rich Text)
- [ ] Загрузка видео/аудио
- [ ] Создание тестов/квизов

#### Фаза 3: Просмотр контента (1-2 недели)

**Задачи:**
- [ ] Страница каталога курсов (`/catalog`)
...
#### Фаза 4: Подписки и уведомления (2-3 недели)
...
#### Фаза 5: Калькуляторы (2-3 недели)
...
### 14.3 Технический долг

**Необходимо исправить:**

| Проблема | Приоритет | Описание |
|----------|-----------|----------|
| navigate() в рендере | Средний | ProfileSettingsPage вызывает navigate вне useEffect |
| Загрузка аватара | Низкий | UI есть, логика не реализована |
| Индексы в БД | Высокий | Добавить индексы для AuthorApplication |
| Валидация на бэкенде | Средний | Добавить Zod-схемы для author routes |

### 14.4 Рекомендации по улучшению

**Краткосрочные:**
1. Добавить уведомления об ошибках на фронтенде
2. Улучшить валидацию форм
3. Добавить loading states для всех API-запросов
4. Оптимизировать запросы к БД (include vs select)

**Среднесрочные:**
1. Кэширование ответов API
2. Rate limiting для защищённых роутов
